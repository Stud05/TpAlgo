...
public class ShortToStringMapTest{
  static class S2SMap {
    ...
    // ------------------------------------------------------------
    public S2SMap () {}
    public void    put(short key, String img) { t.put(key, img);         } 
    public String  get(short key)             { return t.get(key);       }
    public void    remove(short e)            { t.remove(e);             }
    public boolean containsKey(short k)       { return t.containsKey(k); }
    public boolean isEmpty()                  { return size() == 0;      }
    public int     size()                     { return t.size();         }
    public void    union(S2SMap m)            { t.putAll(m.t);           }
    // ------------------------------------------------------------
  ...
  static boolean areSetEqual(ShortToStringMap s, S2SMap g) {
    int l = 0;
    short lastKey=10;
    if (g.size()>0) lastKey=g.t.lastKey();
