...
public class BitSetOfShortsTest {
  ...
  static boolean areSetEqual(BitSetOfShorts s, BitSet bs) {
    ...
    OptionalInt problem=bs.stream()
        .filter(e -> !s.contains((short)(e-D)))
        .findFirst();
    if (problem.isPresent()) {
      System.out.println("bug at element : " + problem.getAsInt());
      return false;
    }
    BitSetOfShortsItr itr = s.iterator();
    int n=0;
    while (itr.hasMoreElements()) {
      n++;
      short e = itr.nextElement();
      if (!bs.get(e + D)) {
        System.out.println("bug at iterated element : " + e);
        return false;
      }
    }
    if(n != s.size()) {
      System.out.println("bug: the iterator does not give all elements");
      return false;
    }
