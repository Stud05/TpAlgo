...
public class MyBitSetTest {
  ...
  public static void testSet(int n) {
    ...
    testAddRm(s, bs, r, n);
    int nMax = 1000, nRepetitions=20;
    testOr (r, nMax, nRepetitions);
    testAnd(r, nMax, nRepetitions);
    testXor(r, nMax, nRepetitions);
    testAndOrXorSpecial(r, nMax);
  }
  
  public static void testOr(Random r, int n, int nRepetitions) {
    int i = 0;
  ...
  public static void testAnd(Random r, int n, int nRepetitions) {
    int i = 0;
    for (i = 0; i < nRepetitions; i++) {
      MyBitSet s1 = new MyBitSet();
      MyBitSet s2 = new MyBitSet();
  ...
  public static void testXor(Random r, int n, int nRepetitions) {
    int i = 0;
    for (i = 0; i < nRepetitions; i++) {
      MyBitSet s1 = new MyBitSet();
      MyBitSet s2 = new MyBitSet();
  ...
  public static void testAndOrXorSpecial(Random r, int n) {
    MyBitSet s1 = new MyBitSet();
    BitSet bs1 = new BitSet();
    testAddRm(s1, bs1, r, n);
    s1.and(s1);
    s1.or(s1);
    if (!areSetEqual(s1, bs1))
      throw new RuntimeException("Error in a.and-or(a)  !");
    s1.xor(s1);
    if(s1.cardinality() != 0)
  ...
  public static void testAddRm(MyBitSet s, BitSet bs, Random r, int n) {
    ...
    while(a<n) {
      ...
      for(i=0;i<a;i++) {
        int j=r.nextInt(n); s.set(j); bs.set(j);
      }
      a = (int)((a+1)*1.8);
      //System.out.print(".");
    }
