...
public class TurtlePanel extends JComponent {
  ...
  private BufferedImage     buffer;       // Internal image
  private Graphics2D        bg2d;         // Graphical context associated to
                                          // internal image
  private boolean           bReady;       // Buffer ready to paint on screen
  ...
  //   initY  : Initial pen position (y-axis) [pixel]
  //----------------------------------------------------------------------------
  public static TurtlePanel create(String title, int     width,
                                                 int     height,
                                                 float   initX, 
                                                 float   initY,
                                                 boolean exitOnClose)  {
    
    TurtleFrame  tf = new TurtleFrame(title,
                                      width, height, initX, initY, exitOnClose);
    return tf.tPanel;
  }
  ...
  public void init(BufferedImage image) {
    ...
    if (bufferWidth  < MIN_BUFFER_WIDTH)  bufferWidth  = MIN_BUFFER_WIDTH;
    if (bufferHeight < MIN_BUFFER_HEIGHT) bufferHeight = MIN_BUFFER_HEIGHT;
  ...
  //----------------------------------------------------------------------------
  public void drawCurve(float fromX, float fromY,
                        float ctrlX, float ctrlY, float toX,   float toY)                              {
    
    QuadCurve2D.Float curve = new QuadCurve2D.Float(fromX, fromY,
                                                    ctrlX, ctrlY,
                                                    toX,   toY);
    setCurrentStroke();
    bg2d.draw(curve);
  ...
  // Draw ellipse
  //----------------------------------------------------------------------------
  public void drawEllipse(float centerX, float centerY,
                          float width,   float height, boolean filled)         {
    
    Ellipse2D.Float ellipse = new Ellipse2D.Float(centerX-width/2,
                                                        centerY-height/2,
                                                        width, height);
    if (filled) {
      bg2d.fill(ellipse);
  ...
  // Draw rectangle
  //----------------------------------------------------------------------------
  public void drawRectangle(float upperLeftX, float upperLeftY,
                            float width,      float height, boolean filled)    {
    
    Rectangle2D.Float rectangle = new Rectangle2D.Float(upperLeftX,
                                                              upperLeftY,
                                                              width, height);
    if (filled) {
      bg2d.fill(rectangle);
  ...
  //----------------------------------------------------------------------------
  public BufferedImage cloneCurrentImage(int fromX, int fromY,
  ...
  public String toString() {
    return "Turtle position: ["+x+", "+y +"]   orientation: " + orientation + '\n'
  ...
  protected void paintComponent(Graphics g) {
    ...
    if (bReady) {
      ...
      if (TEST_MODE) System.out.println("> paintComponent() : Buffer not ready");
    }
  }
  ...
  private BasicStroke createStroke(float[] dashPattern) {
    BasicStroke stroke = new BasicStroke(penSize, 
  ...
  // Draw a grid
  //----------------------------------------------------------------------------
  public void drawGrid(float fromX, float toX, float xInterval,
                       float fromY, float toY, float yInterval) {
    ...
    for (float x=fromX; x<toX; x+=xInterval) {
      if (abs(x)<1e-5) bg2dc.setPaint(GRID_0_0_COLOR);
      else             bg2dc.setPaint(GRID_COLOR);
      bg2dc.draw(new Line2D.Float(x, fromY, x, toY));
    }

    //--- Horizontal lines
    for (float y=fromY; y<toY; y+=yInterval) {
      if (abs(y)<1e-5) bg2dc.setPaint(GRID_0_0_COLOR);
      else             bg2dc.setPaint(GRID_COLOR);
  ...
  private static class PanelResizeAdapter extends ComponentAdapter {
    ...
    public void componentResized(ComponentEvent cEvent) {
      if (panel.buffer==null) return;  // If called before buffer initialized
  ...
  private static class PanelMouseAdapter extends MouseAdapter {
    ...
    public void mousePressed(MouseEvent mEvent) {
      panel.repaint();
      if (TEST_MODE) System.out.println("> mousePressed() : Repainted");
    }
  }
  ...
  public static class TurtleFrame extends JFrame {
    ...
    //   initY  : Initial pen position (y-axis) [pixel]
    //--------------------------------------------------------------------------
    /* package*/ TurtleFrame(final String  title,
                             final int     width,  final int   height,
                             final float   initX,  final float initY,
                             final boolean exitOnClose)                  {
    
    URL urlIcon = getClass().getResource("/resources/Turtle_Icon.png");
      if (urlIcon != null) setIconImage(new ImageIcon(urlIcon).getImage());
